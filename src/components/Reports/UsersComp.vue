<template>
    <v-card flat>
      <v-row>
        <v-col md="2">
          <v-select :items="userStatusList" v-model="userStatusValue" label="User Role"></v-select>
        </v-col>
    ...
    methods: {
      ...
      const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0);
        ...
        const requestOptions = {
          ...
          body: JSON.stringify({ role: "Campaign Manager" })
        };
        fetch(
          "http://localhost:3000/api/user/getAllUsers", requestOptions)
          .then(async (response) => {
