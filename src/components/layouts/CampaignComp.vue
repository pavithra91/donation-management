        ...
        <v-card max-width="344">
          ...
          <v-row>
            ...
            <v-col cols="2">
              ...
              <v-icon @click="removeCampaignFromWatchList(item.id)"
                >mdi-heart</v-icon>
            </v-btn>
          ...
          <v-row>
            <v-col class="mx-4">
              <v-progress-linear
                :value="(item.raiedAmount / item.goalAmount) * 100"
                height="8"
                color="#09cc7f"
              >
              </v-progress-linear>
          ...
          <v-layout row class="mx-0 pa-0">
            <v-col cols="6">
              ...
              <label class="font-weight-medium"
                >Rs. {{ item.raiedAmount.toLocaleString() }}</label
              >
            </v-col>
            <v-col cols="6" class="text-right">
              <label class="font-weight-medium">Goal : </label>
              <label class="font-weight-medium"
                >Rs. {{ item.goalAmount.toLocaleString() }}</label
              >
            </v-col>
          </v-layout>
        </v-card>
